...
大家好，欢迎查看我们`Stable Diffusion`系列教程`Comfy UI`篇。我们将为大家介绍如何在`LooPIN`上一键部署`Stable Diffusion`的`Comfy UI`，以及初步体验使用粘土风格滤镜。`Comfy UI`采用节点化操作，可控性更高，更容易复现和微调出图结果，熟练之后还可以定制个性化工作流。
...
本期视频介绍了如何在LooPIN去中心化GPU云服务器上部署Stable Diffusion的ComfyUI，并初步体验了加载使用粘土风格模型的效果。在后续的课程中，我们将深入探讨ComfyUI的更多玩法和应用。
